...
脚本体。脚本体运行在 async 上下文里——直接用 `await`——它的顶层 `return` 就是 workflow
的结果。原语都是**注入的全局**：**不要** import 它们。
...
- `args` 是你用 `--args` 传入的输入（解析后的 JSON，或一段原始字符串）。
...
| `schema` | 一个原始 JSON Schema 对象，作为 `agent(..., { schema })` 传入。 |

`agent` 的 `opts`：`{ label?, phase?, schema?, adapter? }`（v1；`model` / `agentType` /
`isolation` 可传入，但保留到 v1.5+）。完整参考：
...
- 默认每个 agent 在工作树的一个隔离副本里运行（`workspaceMode: "copy"`）；你真实的工作
  树不会被改动。只读 / 分析型 workflow 用 `inplace`。
