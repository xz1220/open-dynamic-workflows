...
ODW 已经不只是一个运行时原型,而是进入了 **App + CLI** 一起可用的阶段。最近几个版本主要
推进了这些事:

- **桌面观测台** —— 只读的 Tauri 壳已经可以构建、打包和运行;它内嵌同一套
  `odw serve` 看板,支持 Dock / 托盘展示,退出时也会干净地停掉 sidecar。
- **实时运行监控** —— Activity、Jobs 和 Job detail 页面可以看到活跃运行、不同适配器的
  agent 负载、stale worker、Graph / Logs / Result 标签,也能快速复制 run id 或打开运行目录。
- **Workflow 工作区** —— App 会读取 `.odw/workflows`、`.claude/workflows`、
  `~/.odw/workflows`、`~/.claude/workflows` 里的已保存 workflow,并展示阶段、推断结构、
  源码和最近运行。
- **运行时准确性** —— scheduled agent 的派发和计数更准确,run 记录按客户端需要分桶,看板也
  会诚实展示失联 worker,避免把卡住的任务看成还在正常运行。
- **安装路径打磨** —— 二进制 + skill 的安装脚本、零依赖 Web 看板、桌面 sidecar 打包路径
  已经对齐;agent 装完 ODW 后,就能直接把大任务扇出去跑。
...
<table>
  <tr>
    ...
    <td width="50%">
      ...
      <img src="assets/app-screenshots/workspace.png" alt="Workspace 页面:已保存 workflow、阶段、推断结构和源码" />
    </td>
  </tr>
  <tr>
    <td width="50%">
      <strong>Jobs</strong><br />
      <img src="assets/app-screenshots/jobs.png" alt="Jobs 页面:活跃运行卡片和历史运行表格" />
    </td>
    <td width="50%">
